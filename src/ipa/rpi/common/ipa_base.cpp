				     ...
				     bool &delayedRequestControls)
{
	...
	if (!isMemoryCamera_) {
		...
		/* For memory cams, always force IPAs to run. */
		delayedRequestControls = true;
	}
}
...
void IpaBase::prepareIsp(const PrepareParams &params)
{
	 ...
	 */
	prepareIspFillMetadata(params, ipaContext, embeddedBuffer, hdrChange, delayedRequestControls);
...
void IpaBase::processStats(const ProcessParams &params)
{
	...
	if (processPending_ && frameCount_ >= mistrustCount_) {
		...
		if (it == buffers_.end()) {
			/* Memory cams often don't have or use stats, so don't warn. */
			if (!isMemoryCamera_)
