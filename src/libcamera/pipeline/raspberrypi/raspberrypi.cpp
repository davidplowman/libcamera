...
int PipelineHandlerRPi::start(Camera *camera, const ControlList *controls)
{
	 ...
	 * the IPA.
	 */
	uint32_t ipaCookie = 0;
	data->delayedCtrls_->reset(ipaCookie);
	...
	data->syncTable_.emplace(RPiCameraData::SyncTableEntry{ ipaCookie, 0 });
	data->syncTable_.emplace(RPiCameraData::SyncTableEntry{ ipaCookie + 1, 0 });
